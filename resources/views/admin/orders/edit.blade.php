          ...
          <div class="col-md-6">
            <div class="mb-4">
              ...
              <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                <option value="pending" {{ old('status', $order->status) === 'pending' ? 'selected' : '' }}>รอดำเนินการ
                </option>
                <option value="processing" {{ old('status', $order->status) === 'processing' ? 'selected' : '' }}>
                  กำลังดำเนินการ</option>
                <option value="completed" {{ old('status', $order->status) === 'completed' ? 'selected' : '' }}>สำเร็จ
                </option>
                <option value="canceled" {{ old('status', $order->status) === 'canceled' ? 'selected' : '' }}>ยกเลิก
                </option>
              </select>
              @error('status')
              ...
              @enderror
              <div class="form-text">
                <strong class="text-danger">คำเตือน:</strong> การเปลี่ยนสถานะเป็น "ยกเลิก"
                จะทำให้สินค้าถูกคืนกลับไปเป็นสถานะพร้อมขาย และคืนเงินเข้าสู่บัญชีของผู้ซื้อโดยอัตโนมัติ
              </div>
            </div>
              ...
              @foreach ($order->orderItems as $item)
                <tr>
                  <td>
                    ...
                    <div class="fs-sm text-muted">{{ $item->product->type }} | {{ $item->product->category->name }}
                    </div>
                  </td>
